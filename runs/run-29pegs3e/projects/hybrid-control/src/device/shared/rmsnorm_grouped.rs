//! Thirty-two independent eight-slice RMS groups. HBM transfers explicitly
//! populate EVERY group. Each group reduces all H entries and all-gathers
//! its normalized shards to eight slices, covering all 256 projection slices.
//! This is not a relabel of the single populated group used by baseline RMS.

use furiosa_opt_std::prelude::*;

use crate::axes::{Dummy8, Dummy256, H};
...
pub(crate) fn normalize_from_hbm(
    ...
    type ReducingSlices = m![Dummy256 / 8, H / 480];
    ...
    let mean_square: DmTensor<f32, Chip, Cluster, ReducingSlices, m![1 # 8]> = ctx
        ...
        .commit_trim::<m![1 # 8]>()
        .commit();
    let reduced_mean_square: DmTensor<f32, Chip, Cluster, m![Dummy256 / 8, Dummy8], m![1 # 8]> = ctx
        .main
        .begin(mean_square.view())
        .fetch::<m![1], m![1 # 8]>()
        .collect::<m![1], m![1 # 8]>()
        .vector_init()
        .vector_inter_slice_reduce::<m![Dummy256 / 8, Dummy8], m![1]>(InterSliceReduceOpF32::Add)
        .vector_intra_slice_tag(TagMode::Zero)
    ...
    let rms: DmTensor<f32, Chip, Cluster, m![Dummy256 / 8, Dummy8], m![1 # 8]> = ctx
        .main
    ...
    ctx.main
        ...
        .fetch::<m![1], m![H % 480]>()
        .switch::<Replicated, m![H / 480]>(SwitchConfig::Broadcast1 { slice1: 8, slice0: 1 })
        .collect::<m![H / 8], m![H % 8]>()
        .cast::<bf16, m![H % 8 # 16]>()
        .commit_trim::<m![H % 8]>()
        .commit()

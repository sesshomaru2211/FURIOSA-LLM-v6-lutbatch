...
pub(crate) fn project_up_and_gate(
    ...
    // V6: seven 8-row blocks and a 4-row tail; same columns and rounding.
    // 8 * (3840/16) * sizeof(f32) = 7680 bytes of scale VRF per slice.
    for i in 0..7 {
        let up_weight_scale_vrf: VrfTensor<f32, Chip, Cluster, UpGateRows, m![L % 60 = 8, H / 16]> = ctx
            .sub
            .begin(
                up_weight_scale
                    .view()
                    .tile::<m![L % 60], 8, m![L % 60 = 8 # 60, H / 16]>(8 * i),
            )
            .fetch::<m![L % 60 = 8], m![H / 16]>()
            .fetch_cast::<f32>()
            .collect::<m![L % 60 = 8, H / 16 / 8], m![H / 16 % 8]>()
            .to_vrf();

        // Decode the raw F4 block in the fetch stream; no intermediate F8 DM tensor.
        let up_weight: DmTensor<bf16, Chip, Cluster, UpGateRows, m![L % 60 = 8, H]> = ctx
            .main
            .begin(
                up_weight_packed
                    .view()
                    .tile::<m![L % 60], 8, m![L % 60 = 8 # 60, H]>(8 * i),
            )
            .fetch::<m![L % 60 = 8, H / 32], m![H % 32]>()
            .fetch_table_lookup::<f8e4m3>()
            .fetch_cast::<f32>()
            .collect::<m![L % 60 = 8, H / 8], m![H % 8]>()
            .vector_init()
            .vector_intra_slice_tag(TagMode::Zero)
            .vector_narrow_split::<m![L % 60 = 8, H / 4], m![H % 4]>()
            .vector_fp_binary(FpBinaryOp::MulF(FpMulAlu::Mul0), &up_weight_scale_vrf)
            .vector_widen_concat::<m![L % 60 = 8, H / 8], m![H % 8]>()
            .vector_final()
            .cast::<bf16, m![H % 8 # 16]>()
            .commit_trim::<m![H % 8]>()
            .commit();
        ...
        ctx.main
            ...
            .fetch::<m![L % 60 = 8, H / 16], m![H % 16]>()
            .collect::<m![L % 60 = 8, H / 16], m![H % 16]>()
            .contract_outer::<m![L % 60 = 8, H / 32], m![H % 32], _, _, _>(&x_trf)
            .contract_packet::<m![1]>()
            .contract_time::<m![L % 60 = 8]>()
            .contract_lane::<m![L % 60 = 8], m![1 # 8]>(LaneMode::Interleaved)
            .cast::<bf16, m![1 # 16]>()
            .transpose::<m![L % 60 = 8 / 4], m![L % 60 = 8 % 4 # 16]>()
            .commit_trim::<m![L % 60 = 8 % 4]>()
            .commit_view(up.view_mut().tile::<m![L % 60], 8, m![L % 60 = 8 #{!} 60]>(8 * i));
    }
    {
        let up_weight_scale_vrf: VrfTensor<f32, Chip, Cluster, UpGateRows, m![L % 60 = 4, H / 16]> = ctx
            ...
            .begin(
                up_weight_scale
                    .view()
                    .tile::<m![L % 60], 4, m![L % 60 = 4 # 60, H / 16]>(56),
            )
        ...
        let up_weight: DmTensor<bf16, Chip, Cluster, UpGateRows, m![L % 60 = 4, H]> = ctx
            ...
            .begin(
                up_weight_packed
                    .view()
                    .tile::<m![L % 60], 4, m![L % 60 = 4 # 60, H]>(56),
            )
        ...
        ctx.main
            ...
            .commit_view(up.view_mut().tile::<m![L % 60], 4, m![L % 60 = 4 #{!} 60]>(56));
    }
    ...
    // 8 * (3840/16) * sizeof(f32) = 7680 bytes of scale VRF per slice.
    for i in 0..7 {
        let gate_weight_scale_vrf: VrfTensor<f32, Chip, Cluster, UpGateRows, m![L % 60 = 8, H / 16]> = ctx
            .sub
            .begin(
                gate_weight_scale
                    .view()
        ...
        // Decode the raw F4 block in the fetch stream; no intermediate F8 DM tensor.
        let gate_weight: DmTensor<bf16, Chip, Cluster, UpGateRows, m![L % 60 = 8, H]> = ctx
            .main
            .begin(
                gate_weight_packed
                    .view()
        ...
        ctx.main
            ...
            .transpose::<m![L % 60 = 8 / 4], m![L % 60 = 8 % 4 # 16]>()
            .commit_trim::<m![L % 60 = 8 % 4]>()
            .commit_view(gate.view_mut().tile::<m![L % 60], 8, m![L % 60 = 8 #{!} 60]>(8 * i));
    }
    {
        ...
        ctx.main
            ...
            .transpose::<m![1], m![L % 60 = 4 # 16]>()
            .commit_trim::<m![L % 60 = 4]>()
            .commit_view(gate.view_mut().tile::<m![L % 60], 4, m![L % 60 = 4 #{!} 60]>(56));
    }
...
pub(crate) fn project_down(
    ...
    // V6: seven 16-row blocks and an 8-row tail cover exactly 120 rows.
    // 16 * (1920/16) * sizeof(f32) = 7680 bytes of scale VRF per slice.
    // Preserve the original 8-slice f32 reduction before BF16 rounding.
    for i in 0..7 {
        let down_weight_scale_vrf: VrfTensor<f32, Chip, Cluster, DownRowsByColumns, m![H % 120 = 16, L / 16 % 120]> = ctx
            .sub
            .begin(
                down_weight_scale
                    .view()
                    .tile::<m![H % 120], 16, m![H % 120 = 16 # 120, L / 16 % 120]>(16 * i),
            )
            .fetch::<m![H % 120 = 16], m![L / 16 % 120]>()
            .fetch_cast::<f32>()
            .collect::<m![H % 120 = 16, L / 16 / 8 % 15], m![L / 16 % 8]>()
            .to_vrf();

        // Decode the raw F4 block in the fetch stream; no intermediate F8 DM tensor.
        let down_weight: DmTensor<bf16, Chip, Cluster, DownRowsByColumns, m![H % 120 = 16, L % 1920]> = ctx
            .main
            .begin(
                down_weight_packed
                    .view()
                    .tile::<m![H % 120], 16, m![H % 120 = 16 # 120, L % 1920]>(16 * i),
            )
            .fetch::<m![H % 120 = 16, L / 32 % 60], m![L % 32]>()
            .fetch_table_lookup::<f8e4m3>()
            .fetch_cast::<f32>()
            .collect::<m![H % 120 = 16, L / 8 % 240], m![L % 8]>()
            .vector_init()
            .vector_intra_slice_tag(TagMode::Zero)
            .vector_narrow_split::<m![H % 120 = 16, L / 4 % 480], m![L % 4]>()
            .vector_fp_binary(FpBinaryOp::MulF(FpMulAlu::Mul0), &down_weight_scale_vrf)
            .vector_widen_concat::<m![H % 120 = 16, L / 8 % 240], m![L % 8]>()
            .vector_final()
            .cast::<bf16, m![L % 8 # 16]>()
            .commit_trim::<m![L % 8]>()
            .commit();
        ...
        ctx.main
            ...
            .fetch::<m![H % 120 = 16, L / 16 % 120], m![L % 16]>()
            .collect::<m![H % 120 = 16, L / 16 % 120], m![L % 16]>()
            .contract_outer::<m![H % 120 = 16, L / 32 % 60], m![L % 32], _, _, _>(&x_trf)
            .contract_packet::<m![1]>()
            .contract_time::<m![H % 120 = 16]>()
            .contract_lane::<m![H % 120 = 16], m![1 # 8]>(LaneMode::Interleaved)
            .vector_init()
            .vector_inter_slice_reduce::<DownRows, m![H % 120 = 16]>(InterSliceReduceOpF32::Add)
            .vector_final()
            .cast::<bf16, m![1 # 16]>()
            .transpose::<m![H % 120 = 16 / 4], m![H % 120 = 16 % 4 # 16]>()
            .commit_trim::<m![H % 120 = 16 % 4]>()
            .commit_view(down.view_mut().tile::<m![H % 120], 16, m![H % 120 = 16 #{!} 120]>(16 * i));
    }
    {
        let down_weight_scale_vrf: VrfTensor<f32, Chip, Cluster, DownRowsByColumns, m![H % 120 = 8, L / 16 % 120]> = ctx
            .sub
            .begin(
                down_weight_scale
                    .view()
                    .tile::<m![H % 120], 8, m![H % 120 = 8 # 120, L / 16 % 120]>(112),
            )
            .fetch::<m![H % 120 = 8], m![L / 16 % 120]>()
            .fetch_cast::<f32>()
            .collect::<m![H % 120 = 8, L / 16 / 8 % 15], m![L / 16 % 8]>()
            .to_vrf();

        // Decode the raw F4 block in the fetch stream; no intermediate F8 DM tensor.
        let down_weight: DmTensor<bf16, Chip, Cluster, DownRowsByColumns, m![H % 120 = 8, L % 1920]> = ctx
            .main
            .begin(
                down_weight_packed
                    .view()
                    .tile::<m![H % 120], 8, m![H % 120 = 8 # 120, L % 1920]>(112),
            )
            .fetch::<m![H % 120 = 8, L / 32 % 60], m![L % 32]>()
            .fetch_table_lookup::<f8e4m3>()
            .fetch_cast::<f32>()
        ...
        ctx.main
            ...
            .collect::<m![H % 120 = 8, L / 16 % 120], m![L % 16]>()
            .contract_outer::<m![H % 120 = 8, L / 32 % 60], m![L % 32], _, _, _>(&x_trf)
            .contract_packet::<m![1]>()
            .contract_time::<m![H % 120 = 8]>()
            .contract_lane::<m![H % 120 = 8], m![1 # 8]>(LaneMode::Interleaved)
            .vector_init()
            .vector_inter_slice_reduce::<DownRows, m![H % 120 = 8]>(InterSliceReduceOpF32::Add)
            .vector_final()
            .cast::<bf16, m![1 # 16]>()
            .transpose::<m![H % 120 = 8 / 4], m![H % 120 = 8 % 4 # 16]>()
            .commit_trim::<m![H % 120 = 8 % 4]>()
            .commit_view(down.view_mut().tile::<m![H % 120], 8, m![H % 120 = 8 #{!} 120]>(112));
    }

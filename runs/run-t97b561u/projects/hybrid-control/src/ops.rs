...
pub fn sliding_project_qkv(
    ...
    let x = shared::rmsnorm_grouped::normalize_from_hbm(ctx, x, input_rms_weight);
...
pub fn sliding_attention_output(
    ...
    let x: DmTensor<bf16, Chip, Cluster, Slice, m![H]> =
        sliding::projection::project_output_hbm(ctx, x, o_weight, o_weight_scale);
...
pub fn decoder_feedforward(
    ...
    let residual: DmTensor<bf16, Chip, Cluster, Slice, m![H]> = residual_hbm.to_dm(&mut ctx.tdma);
    // Replicate the input across real eight-slice RMS groups, as in the passing QKV path.
    let x = shared::rmsnorm_grouped::normalize_from_hbm(ctx, residual_hbm, pre_ff_rms_weight);
